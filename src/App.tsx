  ...
  const currentPhase = cycleInfo?.phase || 'unknown'
   ...
   return (
     <div className="min-h-screen bg-white">
       {activeTab === 'calendar' && mainContent}
       {activeTab === 'stats' && statsContent}
       {activeTab === 'settings' && settingsContent}
       {activeTab === 'record' && (
         <div className="pb-20 px-4 py-6">
           <h2 className="text-lg font-bold text-gray-800 mb-4">{t('nav.record')}</h2>
           <p className="text-sm text-gray-400">{t('dailyLog.clickToEdit')}</p>
           {currentLog.date && (
             <DailyLog
               log={currentLog}
               phase={currentPhase}
