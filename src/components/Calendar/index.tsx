...
const PHASE_KEYS: CyclePhase[] = ['menstrual', 'follicular', 'ovulation', 'luteal']
...
export default function Calendar({ periods, onDateClick }: Props) {
  ...
  const days = useMemo(() => {
    const result: { date: Date; phase: CyclePhase; isPredicted: boolean; isPeriodDay: boolean }[] = []
    for (let d = 1; d <= daysInMonth; d++) {
      const date = new Date(viewYear, viewMonth, d)
      const { phase, isPredicted } = getPhaseForDateWithInfo(date, periods)
      const dateStr = formatDateStr(date)
      const isPeriodDay = periods.some(p => dateStr >= p.startDate && dateStr <= p.endDate)
      result.push({ date, phase, isPredicted, isPeriodDay })
    }
    return result
  ...
  return (
    <div className="px-4 py-2">
      ...
      <div className="grid grid-cols-7">
        ...
        {days.map(({ date, phase, isPredicted, isPeriodDay }) => {
          ...
          return (
            ...
            >
              <span className={`text-sm ${isT ? 'font-bold text-luna-pink' : 'text-gray-700'} ${isPeriodDay ? 'relative z-10' : ''}`}>{date.getDate()}</span>
              {isPeriodDay && (
                <div className="absolute inset-1 rounded-full ring-2 ring-luna-pink ring-offset-1" />
              )}
              {!isPeriodDay && hasData && (isPredicted ? (
              ...
              {isT && !hasData && !isPeriodDay && <div className="w-1.5 h-1.5 rounded-full mt-0.5 bg-gray-300" />}
            </button>
          )
        })}
      </div>
      ...
      {/* Legend */}
      <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 justify-center text-xs text-gray-500">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full border-2 border-dashed" style={{ borderColor: '#FF6B9D' }} />
          <span>{t('legend.predicted')}</span>
        </div>
      </div>
    </div>
  )
